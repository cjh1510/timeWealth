$(() => {
    ...
    function renderUI(orderData) {
        ...
        orderData.forEach(data => {
            ...
            let listHtml = data.goods.map(item => {
                ...
                <li class="list_con">
                  <div class="list_img"><img src=${item.src1} alt=""></div>
    ...
    function dataTool(data) {
        let arr = [];
        data.forEach(item => {
            let result = arr.filter((ele) => ele.store == item.author);
            if (result.length == 0) {
                arr.push({ store: item.author, goods: [] });
            }
        })
